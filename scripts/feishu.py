...
def post_payload_with_response(payload: dict[str, Any], *, error_prefix: str) -> FeishuResponse:
    webhook = os.getenv("FEISHU_WEBHOOK", "").strip()
    if not webhook:
        return FeishuResponse(False, None, "FEISHU_WEBHOOK 未配置", "")
    ...
    for attempt in range(attempts):
        ...
        if result.get("code") in (0, None):
            return FeishuResponse(True, result.get("code"), str(result.get("msg") or result.get("message") or ""), body)
...
def post_payload(payload: dict[str, Any], *, error_prefix: str) -> bool:
    return post_payload_with_response(payload, error_prefix=error_prefix).ok
...
def send_card_with_response(card: dict[str, Any]) -> FeishuResponse:
    webhook = os.getenv("FEISHU_WEBHOOK", "").strip()
    if not webhook:
        return FeishuResponse(False, None, "FEISHU_WEBHOOK 未配置", "")
    ...
    if secret:
        ...
        payload["timestamp"] = timestamp
        payload["sign"] = sign(secret, timestamp)
    return post_payload_with_response(payload, error_prefix="飞书卡片发送失败")
